...
export class MiniCartComponentService {
  ...
  // function to get the of the cart item's count
  getEntriesLength(): Observable<number> {    
    return this.activeCartRequired().pipe(
      switchMap((activeCartRequired) => {
        if (activeCartRequired) {
          return this.activeCartFacade.getEntries().pipe(
            filter((entries: AdnocOrderEntry[]) => !!entries),
            map((entries: AdnocOrderEntry[]) => entries.length)
          );
        } else {
          return of(0);
        }
