...
import { AdnocQuoteConnector } from '../connectors/adnocQuote.connector';
import {
  CartUtilsService,
  QuoteCartService,
...
import { MultiCartFacade } from '../../../cart/base/root/facade/adnoc-multi-cart.facade';
...
export class AdnocQuoteService implements AdnocQuoteFacade {
  protected userIdService = inject(UserIdService);
  protected authService = inject(AdnocAuthService);
  protected quoteConnector = inject(AdnocQuoteConnector);
  ...
  performQuoteAction(
    ...
    return this.performQuoteActionCommand.execute({
      quote,
      quoteAction,
      payLoads: payLoad,
    });
  ...
  cartToQuote(payload: ICartQuotes, cartId: string): Observable<OccQuote> {
    let url = this.OccEndpointsService.buildUrl('cartToQuotes', {
      urlParams: {
        cartId,
      },
    });
    return this.http.post<OccQuote>(url, payload);
  }
}
