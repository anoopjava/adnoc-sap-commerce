      ...
      combineLatest([
        ...
        .subscribe(([isQuoteCartActive, cartQuoteId]) => {
          if (
            ...
            (isQuoteCartActive && cartQuoteId === quoteId)
          ) {
            const removeActions = [QuoteActionType.EDIT, QuoteActionType.CANCEL];
            removeActions.forEach((action) => {
              const idx = clonedActionList.indexOf(action);
              if (idx > -1) {
                clonedActionList.splice(idx, 1);
              }
            });
          }
        });
