...
public class AdnocPartialOrderCancellationSmsAction extends AbstractAdnocB2bGenerateSmsAction
{
    private static final Logger LOG = LogManager.getLogger(AdnocPartialOrderCancellationSmsAction.class.getName());

    public static final String ADNOC_ORDER_PARTIALLY_CANCELLED_SMS_BODY = "adnoc.order.partially.cancelled.sms.body";
    ...
    {
        LOG.debug("appEvent=AdnocOrderPartialCancelledSmsAction,executing SMS action for process:{}", businessProcessModel.getCode());
        executeSmsAction(businessProcessModel, ADNOC_ORDER_PARTIALLY_CANCELLED_SMS_BODY);

        LOG.debug("appEvent=AdnocOrderPartialCancelledSmsAction,executed SMS action for process:{}", businessProcessModel.getCode());
    ...
    protected void executeSmsAction(final BusinessProcessModel businessProcessModel, final String adnocOrderPartialSmsBody)
    {
        if (businessProcessModel instanceof OrderProcessModel)
        {
            ...
            if (orderProcessModel.getOrder() instanceof OrderModel)
            {
                ...
                LOG.info("appEvent=AdnocOrderPartialCancelledSmsAction,SMS sent successfully for partially completed order:{}", orderProcessModel.getCode());
            }
        }
