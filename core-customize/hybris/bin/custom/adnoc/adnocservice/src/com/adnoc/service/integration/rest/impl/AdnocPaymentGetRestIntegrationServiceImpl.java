...
public class AdnocPaymentGetRestIntegrationServiceImpl implements AdnocRestIntegrationService
{

    private static final Logger LOG = LogManager.getLogger(AdnocPaymentGetRestIntegrationServiceImpl.class);
    ...
    public <T> T restIntegration(final String destinationId, final String destinationTarget, final Object requestObject, final Class<T> responseType)
    {
        ...
        try
        {
            if (consumedDestinationModel.getCredential() instanceof final ConsumedOAuthCredentialModel consumedOAuthCredentialModel)
            {
                ...
                final String updatedUrl = String.format("%s/%s", consumedDestinationModel.getUrl(), requestObject);
                final T responseObject = restTemplate.exchange(updatedUrl, HttpMethod.GET, new HttpEntity<>(headers), responseType).getBody();
