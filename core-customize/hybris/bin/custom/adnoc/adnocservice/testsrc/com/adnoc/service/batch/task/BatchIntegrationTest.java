 ...
 */
package com.adnoc.service.batch.task;
...
public class BatchIntegrationTest extends ServicelayerTest
{
	...
	private static final String ADNOC_CATALOG_NAME = "adnocProductCatalog";
	...
	public void setUp() throws Exception
	{
		...
		ServicelayerTest.importCsv("/adnocservice/test/testBatch.impex", "utf-8");
	...
	public void testBasicProduct() throws Exception
	{
		...
		Assert.assertEquals(ADNOC_CATALOG_NAME, product.getCatalogVersion().getCatalog().getId());
	}
	...
	public void testPrice() throws Exception
	{
		...
		try
		{
			final ProductModel product = processFile(productId, "price-", new FileContent()
			{
				...
				{
					writer.print("1231");
					writer.print(BatchIntegrationTest.SEPARATOR);
					writer.println("EUR");
				}
			});
			...
			Assert.assertEquals(ADNOC_CATALOG_NAME, prize.getCatalogVersion().getCatalog().getId());
		}
		finally
		{
	...
	public void testMedia() throws Exception
	{
		...
		final Set<String> containerFormats = new HashSet<>();
		...
		Assert.assertEquals(ADNOC_CATALOG_NAME, container.getCatalogVersion().getCatalog().getId());
	}
	...
	protected void verifyMedia(final MediaModel media, final String format)
	{
		...
		Assert.assertEquals(ADNOC_CATALOG_NAME, media.getCatalogVersion().getCatalog().getId());
	...
	protected BatchHeader createHeader(final File file)
	{
		...
		header.setCatalog(ADNOC_CATALOG_NAME);
		return header;
	}
	...
	protected void createFile(final File file, final Long productId, final FileContent fileContent) throws IOException
	{
		...
		try (final PrintWriter prWriter = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), CSVConstants.DEFAULT_ENCODING))))
		{
			prWriter.print(productId);
			prWriter.print(SEPARATOR);
			fileContent.writeContent(prWriter);
		} finally
		{
	...
	protected static class ProductContent implements FileContent
	{
		...
		public void writeContent(final PrintWriter writer) throws IOException
		{
			...
			writer.print("name");
			writer.print(BatchIntegrationTest.SEPARATOR);
			writer.print(BatchIntegrationTest.DESCRIPTION_TEST_VALUE);
			writer.print(BatchIntegrationTest.SEPARATOR);
			writer.print("ean");
			writer.print(BatchIntegrationTest.SEPARATOR);
			...
			writer.print(BatchIntegrationTest.SEPARATOR);
			writer.print(BatchIntegrationTest.UNIT_TEST_VALUE);
			writer.print(BatchIntegrationTest.SEPARATOR);
