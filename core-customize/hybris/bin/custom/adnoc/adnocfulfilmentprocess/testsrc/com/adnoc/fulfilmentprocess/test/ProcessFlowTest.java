...
import com.adnoc.fulfilmentprocess.test.actions.TestActionTemp;
...
public class ProcessFlowTest extends HybrisJUnit4Test
{
	...
	public static void prepare() throws Exception
	{
		...
		xmlReader.loadBeanDefinitions(
				new ClassPathResource("/adnocfulfilmentprocess/test/adnocfulfilmentprocess-spring-test.xml"));
		xmlReader
				.loadBeanDefinitions(new ClassPathResource("/adnocfulfilmentprocess/test/process/order-process-spring.xml"));
		xmlReader.loadBeanDefinitions(
				new ClassPathResource("/adnocfulfilmentprocess/test/process/consignment-process-spring.xml"));
		...
		definitonFactory.add("classpath:/adnocfulfilmentprocess/test/process/order-process.xml");
		definitonFactory.add("classpath:/adnocfulfilmentprocess/test/process/consignment-process.xml");
		LOG.warn("loaded 'order-process-test':" + definitonFactory.getProcessDefinition(new ProcessDefinitionId("order-process-test")) + " in factory "
				+ definitonFactory);
		...
		commandFactoryReg.setCommandFactoryList(List.of((CommandFactory) appCtx.getBean("mockupCommandFactory")));
	...
	public static void removeProcessDefinitions()
	{
		...
		final XmlBeanDefinitionReader xmlReader = new XmlBeanDefinitionReader((BeanDefinitionRegistry) beanFactory);
		xmlReader.loadBeanDefinitions(
				new ClassPathResource("/adnocfulfilmentprocess/test/adnocfulfilmentprocess-spring-testcleanup.xml"));
	...
	public void testErrorCode() throws InterruptedException
	{
		...
		try
		{
			...
			TestUtils.disableFileAnalyzer("order process", 400);
